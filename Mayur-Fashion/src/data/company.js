export const COMPANY_INFO = {
  name: "Mayur Fashion",
  legalName: "Manohar Dresses",
  brandLabel: "Mayur™",
  tagline: "We Care What You Wear",
  slogan: "From timeless classics to modern trends, explore our diverse collection and elevate your wardrobe today.",
  establishedYear: 1991,
  brandYear: 1991,
  
  stats: [
    { label: "Manufacturing Heritage", value: "35+ Yrs", sub: "Since 1991" },
    { label: "Retail & Wholesale Network", value: "15,000+", sub: "Trusted Partners" },
    { label: "Global Export Reach", value: "30+ Countries", sub: "Worldwide Delivery" },
    { label: "Inclusive Size Range", value: "M to 6XL", sub: "Perfect Fits" },
  ],

  contacts: [
    {
      title: "For Ready to Wear garments",
      highlight: false,
      people: [
        {
          name: "Mayur Peswani",
          role: "Sales and Marketing",
          phone: "+91 99788 31115",
          whatsapp: "919978831115"
        },
        {
          name: "Bharat Peswani",
          role: "Production and Bulk Wholesale",
          phone: "+91 99091 54100",
          whatsapp: "919909154100"
        }
      ]
    },
    {
      title: "Founder & Chairman",
      highlight: false,
      people: [
        {
          name: "Manohar Peswani",
          role: "Founder & Chairman",
          phone: "+91 98253 43225",
          whatsapp: "919825343225"
        }
      ]
    },
    {
      title: "For Dress Material",
      highlight: false,
      people: [
        {
          name: "Amit Peswani",
          role: "Production and bulk wholesale",
          phone: "+91 9727043225",
          whatsapp: "919727043225"
        },
        {
          name: "Girish Punjabi",
          role: "Sales and marketing",
          phone: "+91 9879367580",
          whatsapp: "919879367580"
        }
      ]
    }
  ],

  email: "mayurfashion1991@gmail.com",
  website: "https://mayurfashion.co.in",
  instagram: "https://instagram.com/mayur_fashion_house",
  instagramHandle: "@MAYUR_FASHION_HOUSE",

  locations: [
    {
      type: "Sales Office & Showroom (S.O.)",
      name: "Mayur Fashion Showroom",
      address: "A-35, Ground Floor, Safal 3 Market, Sarangpur, Ahmedabad, Gujarat 380001",
      city: "Ahmedabad, Gujarat",
      mapUrl: "https://maps.app.goo.gl/H43w2d8ZAkVErfXL8"
    },
    {
      type: "Head Office (H.O.)",
      name: "Manohar Dresses Corporate Office",
      address: "A 501, 5th Floor, Iscon Commercial Complex (VIP Market), Omex Mill Compound, Nr. New Cloth Market, Sarangpur, Ahmedabad, Gujarat 380001",
      city: "Ahmedabad, Gujarat",
      mapUrl: "https://maps.app.goo.gl/g8NaiGuVayWc3wmY9"
    }
  ],

  story: {
    title: "The Legacy of Manohar Dresses & Mayur",
    paragraphs: [
      "The journey began in 1991, when Mr. Manohar Peswani and family established Manohar Dresses and registered the celebrated designer brand label 'Mayur' in Ahmedabad, the textile heartland of India, driven by an uncompromising vision for fabric excellence, size inclusivity, and authentic craftsmanship.",
      "Over more than three decades, that foundational passion has expanded into one of India's premier ethnic wear manufacturing houses, trusted by over 15,000 retail boutiques and exporting to 30+ nations worldwide."
    ]
  },

  values: [
    {
      id: "quality",
      title: "Quality Assurance",
      desc: "Meticulous craftsmanship with multi-stage inspection. From thread count to hand-embroidery, every piece meets global export benchmarks.",
      icon: "ShieldCheck"
    },
    {
      id: "sustainability",
      title: "Sustainable Approach",
      desc: "Ethically sourced cottons, pure viscose, and eco-conscious dyeing methods prioritizing skin-friendliness and planetary care.",
      icon: "Leaf"
    },
    {
      id: "inclusivity",
      title: "Stay Ahead of Trends. Grow Your Margins.",
      desc: "We quickly turn viral and trending designs into quality-crafted Indian wear, offering retailers the right products at competitive prices—so you can bring fresh styles to your customers and create greater opportunities for profit.",
      icon: "Sparkles"
    },
    {
      id: "customer",
      title: "Customer Satisfaction",
      desc: "Personalized wholesale services, reliable dispatch timelines, catalog set flexibility, and dedicated direct relationship managers.",
      icon: "HeartHandshake"
    }
  ],

  testimonials: [
    {
      name: "Rajesh Kothari",
      business: "Kothari Silk & Sarees, Mumbai",
      quote: "We have been sourcing Mayur Fashion catalog sets for over 12 years. Their kurti 3-piece sets and sizing up to 6XL are our fastest moving inventory every festive season.",
      rating: 5,
      city: "Mumbai, Maharashtra"
    },
    {
      name: "Sunita Agarwal",
      business: "Roopkala Ethnic Boutique, Jaipur",
      quote: "The fabric quality and intricate threadwork in Sakshi's collection are outstanding. The fit from M to 3XL requires zero customer alterations.",
      rating: 5,
      city: "Jaipur, Rajasthan"
    },
    {
      name: "Tariq Mansoor",
      business: "Al-Noor Textiles LLC, Dubai",
      quote: "Direct wholesale exports with prompt air/sea dispatch. Mayur Fashion is our most reliable manufacturer in Gujarat.",
      rating: 5,
      city: "Dubai, UAE"
    },
    {
      name: "Harpreet Kaur",
      business: "Punjab Heritage Wear, Chandigarh",
      quote: "Unbeatable wholesale rates with premier packaging. Manohar Dresses maintains consistent colorfastness and pure fabric standards.",
      rating: 5,
      city: "Chandigarh, Punjab"
    }
  ]
};
